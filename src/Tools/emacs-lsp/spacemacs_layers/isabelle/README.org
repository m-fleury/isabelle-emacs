...
Emacs)
** Font Configuration
If you have face issue (characters being longer that the space and overlapping with the next character), you can set it with
```
:custom-face
(unicode-tokens-symbol-font-face ((t (:family "Iosevka"))))
```
** Emacs Version
*** Emacs 29

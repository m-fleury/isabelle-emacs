...
Try typing => inside a .thy file now. It should automatically convert the symbol to `\<Rightarrow>`.
If you do not see a fancy symbol, you can add
#+BEGIN_SRC elisp
(add-hook 'isar-mode-hook
	(lambda ()
		(unicode-tokens-mode) (unicode-tokens-mode)))
#+END_SRC
before the matching closing bracket of `(use-package! isar-mode ...`. This is a temporary fix for https://github.com/m-fleury/isabelle-release/issues/62.

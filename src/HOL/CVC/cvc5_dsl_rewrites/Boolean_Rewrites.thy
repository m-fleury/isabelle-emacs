...
lemma [rewrite_bool_double_not_elim]:
  fixes t::"bool"
  shows "NO_MATCH a (undefined t) ==> (\<not> \<not> t) = t"
  by auto
...
lemma [rewrite_bool_eq_true]:
  fixes t::"bool"
  shows "NO_MATCH a (undefined t) ==> (t = True) = t"
  by auto
...
lemma [rewrite_bool_eq_false]:
  fixes t::"bool"
  shows "NO_MATCH a (undefined t) ==> (t = False) = (\<not> t)"
  by auto
...
lemma [rewrite_bool_eq_nrefl]:
  fixes x::"bool"
  shows "NO_MATCH a (undefined x) ==> (x = (\<not> x)) = False"
  by auto
...
lemma [rewrite_bool_impl_false1]:
  fixes t::"bool"
  shows "NO_MATCH a (undefined t) ==> (t \<longrightarrow> False) = (\<not> t)"
  by auto
...
lemma [rewrite_bool_impl_false2]:
  fixes t::"bool"
  shows "NO_MATCH a (undefined t) ==> (False \<longrightarrow> t) = True"
  by auto
...
lemma [rewrite_bool_impl_true1]:
  fixes t::"bool"
  shows "NO_MATCH a (undefined t) ==> (t \<longrightarrow> True) = True"
  by auto
...
lemma [rewrite_bool_impl_true2]:
  fixes t::"bool"
  shows "NO_MATCH a (undefined t) ==> (True \<longrightarrow> t) = t"
  by auto
...
lemma [rewrite_bool_or_true]:
  ...
  shows "NO_MATCH a (undefined xs ys) ==> cvc_list_left (\<or>) xs (cvc_list_right (\<or>) True ys) = True"
  apply (cases ys)
  apply (cases xs)
  subgoal for yss xss 
...
lemma [rewrite_bool_or_false]:
  fixes xs::"bool cvc_ListVar" and ys::"bool cvc_ListVar"
  shows "NO_MATCH a (undefined xs ys) ==> cvc_list_left (\<or>) xs (cvc_list_right (\<or>) False ys) =
...
lemma [rewrite_bool_or_flatten]:
  ...
  shows "NO_MATCH a (undefined xs b ys zs) ==> cvc_list_left (\<or>) xs
...
lemma [rewrite_bool_and_true]:
  fixes xs::"bool cvc_ListVar" and ys::"bool cvc_ListVar"
  shows "NO_MATCH a (undefined xs ys) ==> cvc_list_left (\<and>) xs (cvc_list_right (\<and>) True ys) =
...
lemma [rewrite_bool_and_false]:
  fixes xs::"bool cvc_ListVar" and ys::"bool cvc_ListVar"
  shows "NO_MATCH a (undefined xs ys) ==> cvc_list_left (\<and>) xs (cvc_list_right (\<and>) False ys) = False"
  apply (cases ys)
  apply (cases xs)
  subgoal for yss xss 
    by (simp add: cvc_list_left_transfer cvc_list_right_transfer_op cvc_list_both_transfer_op)
...
lemma [rewrite_bool_and_flatten]:
  fixes xs::"bool cvc_ListVar" and b::"bool" and ys::"bool cvc_ListVar" and zs::"bool cvc_ListVar"
  shows "NO_MATCH a (undefined xs b ys zs) ==> cvc_list_left (\<and>) xs
...
lemma [rewrite_bool_and_conf]:
  ...
  shows "NO_MATCH a (undefined xs w ys zs) ==> cvc_list_left (\<and>) xs
    (w \<and>
...
lemma [rewrite_bool_or_taut]:
  fixes xs::"bool cvc_ListVar" and w::"bool" and ys::"bool cvc_ListVar" and zs::"bool cvc_ListVar"
  shows "NO_MATCH a (undefined xs w ys zs) ==> cvc_list_left (\<or>) xs
    (w \<or>
...
lemma [rewrite_bool_xor_refl]:
  fixes x::"bool"
  shows "NO_MATCH a (undefined x) ==> x [+] x = False"
...
lemma [rewrite_bool_xor_nrefl]:
  fixes x::"bool"
  shows "NO_MATCH a (undefined x) ==> x [+] (\<not> x) = True"
  using xor_def by auto
...
lemma [rewrite_bool_xor_false]:
  fixes x::"bool"
  shows "NO_MATCH a (undefined x) ==> x [+] False = x"
  using xor_def by auto
...
lemma [rewrite_bool_xor_true]:
  fixes x::"bool"
  shows "NO_MATCH a (undefined x) ==> x [+] True = (\<not> x)"
  using xor_def by auto
...
lemma [rewrite_bool_xor_comm]:
  ...
  shows "NO_MATCH a (undefined x) ==> x [+] y = y [+] x"
  using xor_def by auto
...
lemma [rewrite_bool_xor_elim]:
  fixes x::"bool" and y::"bool"
  shows "NO_MATCH a (undefined x) ==> x [+] y = (x \<noteq> y)"
  using xor_def by auto
...
lemma [rewrite_ite_neg_branch]:
  ...
  shows "NO_MATCH a (undefined c x y) ==> (\<not> y) = x \<longrightarrow> (if c then x else y) = (c = x)"
  by auto
...
lemma [rewrite_ite_then_true]:
  ...
  shows "NO_MATCH a (undefined c x) ==> (if c then True else x) = (c \<or> x)"
  by auto
...
lemma [rewrite_ite_else_false]:
  fixes c::"bool" and x::"bool"
  shows "NO_MATCH a (undefined c x) ==> (if c then x else False) = (c \<and> x)"
  by auto
...
lemma [rewrite_ite_then_false]:
  fixes c::"bool" and x::"bool"
  shows "NO_MATCH a (undefined c x) ==> (if c then False else x) = (\<not> c \<and> x)"
  by auto
...
lemma [rewrite_ite_else_true]:
  fixes c::"bool" and x::"bool"
  shows "NO_MATCH a (undefined c x) ==> (if c then x else True) = (\<not> c \<or> x)"
  by auto
...
lemma [rewrite_ite_then_lookahead_self]:
  fixes c::"bool" and x::"bool"
  shows "NO_MATCH a (undefined c x) ==> (if c then c else x) = (if c then True else x)"
  by auto
...
lemma [rewrite_ite_else_lookahead_self]:
  fixes c::"bool" and x::"bool"
  shows "NO_MATCH a (undefined c x) ==> (if c then x else c) = (if c then x else False)"
  by auto

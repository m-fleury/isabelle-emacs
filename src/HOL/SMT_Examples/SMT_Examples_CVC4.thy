(*  Title:      HOL/SMT_Examples/SMT_Examples_cvc4.thy
...
section \<open>Examples for the (smt (cvc4)) binding\<close>

theory SMT_Examples_CVC4
imports Complex_Main
begin

(*external_file \<open>SMT_Examples_CVC4.certs\<close>
 
declare [[smt_certificates = "SMT_Examples_CVC4.certs"]]
declare [[smt_read_only_certificates = false]]*)
declare[[cvc4_options = "--proof-prune-input --proof-format-mode=alethe --simplification=none --dag-thres=0 --lang=smt2 --proof-granularity=dsl-rewrite"]]


section \<open>Propositional and first-order logic\<close>

lemma "True" supply [[smt_trace]] supply [[smt_debug_verit]] by (smt (cvc4))
lemma "p \<or> \<not>p" by (smt (cvc4))
lemma "(p \<and> True) = p" by (smt (cvc4))
lemma "(p \<or> q) \<and> \<not>p \<Longrightarrow> q" by (smt (cvc4))
lemma "(a \<and> b) \<or> (c \<and> d) \<Longrightarrow> (a \<and> b) \<or> (c \<and> d)" by (smt (cvc4))
lemma "(p1 \<and> p2) \<or> p3 \<longrightarrow> (p1 \<longrightarrow> (p3 \<and> p2) \<or> (p1 \<and> p3)) \<or> p1" by (smt (cvc4))
lemma "P = P = P = P = P = P = P = P = P = P" by (smt (cvc4))
...
lemma "(3::int) = 3" by (smt (cvc4))
lemma "(3::real) = 3" by (smt (cvc4))
lemma "(3 :: int) + 1 = 4" by (smt (cvc4))
lemma "x + (y + z) = y + (z + (x::int))" by (smt (cvc4))
lemma "max (3::int) 8 > 5" by (smt (cvc4))
lemma "\<bar>x :: real\<bar> + \<bar>y\<bar> \<ge> \<bar>x + y\<bar>" supply [[smt_trace]] by (smt (cvc4))
lemma "P ((2::int) < 3) = P True" supply[[smt_trace]] by (smt (cvc4))
lemma "x + 3 \<ge> 4 \<or> x < (1::int)" by (smt (cvc4))
...
lemma "let x = (2 :: int) in x + x \<noteq> 5" supply [[smt_trace,smt_nat_as_int]]by (smt (cvc4))
...
lemma "(0 \<le> y + -1 * x \<or> \<not> 0 \<le> x \<or> 0 \<le> (x::int)) = (\<not> False)" by (smt (cvc4))
...
There, it is proved by "arith". (smt (cvc4)) is able to prove this within a fraction
...
lemma "~ (\<exists>x::int. False)" by (smt (cvc4))
lemma "~ (\<exists>x::real. False)" by (smt (cvc4))


lemma "\<forall>x y::int. (x = 0 \<and> y = 1) \<longrightarrow> x \<noteq> y" by (smt (cvc4))
lemma "\<forall>x y::int. x < y \<longrightarrow> (2 * x + 1) < (2 * y)" by (smt (cvc4))
lemma "\<forall>x y::int. x + y > 2 \<or> x + y = 2 \<or> x + y < 2" by (smt (cvc4))
lemma "\<forall>x::int. if x > 0 then x + 1 > 0 else 1 > x" by (smt (cvc4))
lemma "(if (\<forall>x::int. x < 0 \<or> x > 0) then -1 else 3) > (0::int)" by (smt (cvc4))
lemma "\<exists>x::int. \<forall>x y. 0 < x \<and> 0 < y \<longrightarrow> (0::int) < x + y" by (smt (cvc4))
lemma "\<exists>u::int. \<forall>(x::int) y::real. 0 < x \<and> 0 < y \<longrightarrow> -1 < x" by (smt (cvc4))
lemma "\<forall>(a::int) b::int. 0 < b \<or> b < 1" supply[[smt_trace]] by (smt (cvc4))

subsection \<open>Linear arithmetic for natural numbers\<close>

declare [[smt_nat_as_int]]

lemma "2 * (x::nat) \<noteq> 1" by (smt (cvc4))

lemma "a < 3 \<Longrightarrow> (7::nat) > 2 * a" by (smt (cvc4))

lemma "let x = (1::nat) + y in x - y > 0 * x" by (smt (cvc4))
...
lemma "int (nat \<bar>x::int\<bar>) = \<bar>x\<bar>" by (smt (cvc4) int_nat_eq)

definition prime_nat :: "nat \<Rightarrow> bool" where
  "prime_nat p = (1 < p \<and> (\<forall>m. m dvd p --> m = 1 \<or> m = p))"

lemma "prime_nat (4*m + 1) \<Longrightarrow> m \<ge> (1::nat)" by (smt (cvc4) prime_nat_def)
...
lemma True using let_rsp by (smt (cvc4))
lemma "le = (\<le>) \<Longrightarrow> le (3::int) 42" by (smt (cvc4))
lemma "map (\<lambda>i::int. i + 1) [0, 1] = [1, 2]" supply [[smt_trace]] by (smt (cvc4) list.map)
lemma "(\<forall>x. P x) \<or> \<not> All P" by (smt (cvc4))

fun dec_10 :: "int \<Rightarrow> int" where
  "dec_10 n = (if n < 10 then n else dec_10 (n - 10))"

lemma "dec_10 (4 * dec_10 4) = 6" by (smt (cvc4) dec_10.simps)
...
lemma
  ...
  using assms supply [[smt_trace]] by (smt (cvc4) order_trans)
...
lemma
 "eq_set (List.coset xs) (set ys) = rhs"
  ...
  using that by (smt (cvc4, default))
...
lemma "g (Some (3::int)) = g (Some True)" by (smt (cvc4) g1 g2 g3 list.size)
